...
# to ~60s in the worst case) after `az ad app create`. The very next
# Graph call (`az ad sp create`, `az ad app federated-credential ...`)
# can fail with a confusing `JSONDecodeError: Expecting value: line 1
# column 1 (char 0)` while propagation is in flight. We don't poll for
# visibility — that turned out to be brittle on Windows az.cmd shims —
# we just retry the actual operation with backoff and let it converge.
retry_graph_call() {
  local label="$1"
  shift
